	...
	FIT_SETTING=fit_setting,START_PIX=start_pix,weight_index=weight_index,      $
        double=double,fwhm_resolu=fwhm_resolu,fit_rout=fit_rout, $
    ...
    ;max_resolve will be like [0.,0.,10.], the max resolved radius in degree for interferometers
...
while ipass lt maxpasses and notconverged do begin
    ...
    if (not keyword_set(input_p)) or (ipass gt 1) then begin ;XYC's input p
      ...
      ;nlayers is for the whole sphere, != npixel
    	...
    	if max_resolve(nlw) eq 0. then $
    	  unitmass = coreimage(T,density,1.,rout,a,beta1grid(ib),d, $
                   lambda1set(nlw),nlayers,npixels,npixels) else begin
    	  routset = (findgen(nlayers)+1.)*rout/nlayers ;in pc
    	  ;rout is in pc = 0.5*npixels*pixel*!dtor*d
    	  density_part=density*(routset le max_resolve(nlw)*!dtor*d)
    	  ;T_part=T*(routset le max_resolve(nlw)*!dtor*d)
    	  unitmass = coreimage(T,density_part,1.,rout,a,beta1grid(ib),d, $
