...
; Function to be minimized in corefit model.
    ...
    if pwrlaw then r0 = rz else r0 = p(6-alpha_i-beta_i); > 1.e-10 original
    
    if keyword_set(alpha_preset) then p_temp_2=alpha_preset else p_temp_2=p(2-beta_i)
    if keyword_set(beta_preset) then beta1=beta_preset else beta1=p(1)
    ...
    ;T = p(4-alpha_i-beta_i) + (p(3-alpha_i-beta_i)-p(4-alpha_i-beta_i))/(1. + (r/(p(5-alpha_i-beta_i)>1.e-10))^gamma1) original
	...
	for indx_i = 0, nx-1 do begin
	  for indx_j = 0, ny-1 do begin
	    dist1 = sqrt((indx_i-cent_pix)^2+(indx_j-cent_pix)^2) > 1. ;>1 is to get rid of the central 0 problem
	    if ((dist1 gt cent_pix) or (imageset(indx_i,indx_j,n) eq 0.)) then index_array[indx_i,indx_j]=0 $
	       else index_array[indx_i,indx_j]= 1./dist1^qz1; * (alog10(dist1+0.1)-alog10(dist1))/alog10(1.1);
	  endfor
	endfor
	index_array1=float(nx)*float(ny)*index_array/total(index_array); the weird shunxu is to avoid compu error; is normalized
  ...
  if use_sed then begin ;use_sed begin
    ...
    rchisq = double( sum/(wt - n_elements(p)) )
    if finished then return, rchisq

    if limit_range then begin
    if keyword_set(printf_p_init) then begin
        if beta_i eq 0 then begin
	...
	if iterations mod 10 eq 0 then begin
            print,'Iteration',iterations, '     reduced chi squared =',rchisq
            print,'P = ', p
        endif
	phimin = rchisq
    endif
