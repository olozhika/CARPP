...
; Convert intensity units from Jy/beam to Jy/pixel
  convert_unit=0
  if not keyword_set(sxpar(h,'BUNIT')) then convert_unit=1 else begin
    if strpos(sxpar(h,'BUNIT'),'eam') gt -1 $ 
    or strpos(sxpar(h,'BUNIT'),'EAM') gt -1 then begin ;now is Jy/beam
      convert_unit=1
    endif
  endelse
  if convert_unit eq 1 then begin
