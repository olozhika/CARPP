function coreimage_standalone,T,density,cloudmass,rout,a,beta1,d,lambda1,nlayers,nx,ny
...
common dencom,npeak,pc_for_pixel,phicore_quanzhong,full_sphere
    ...
    pc_for_pixel=(rout*2.)/nx
    full_sphere=1

    routset = (findgen(nlayers)+1.)*rout/nlayers; olo's note: start from >0
...
; function of radius.
    ...
    if not full_sphere then begin ; not full sphere
      ...
      mass_temp=total(index_array)/Msun*(pc_for_pixel*pc)^2*mh2 ;mass in Msun
